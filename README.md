...
* `markerInfoFile` - A file listing the markers to be genotyped, must only contain markers from one chromosome. Format for markerInfoFile:
        ...
        chrom startCoordinate endCoordinate repeatMotif numOfRepeatsInRef 1000refBasesBeforeStart 1000refBasesAfterEnd repeatSeqFromRef minFlankLeft minFlankRight
...
To generate the markerInfoFile we have 2 scripts, the first is getRefSeq which retrieves the repeat sequence and 1000 bases on each side from the reference.

Call:
    
    getRefSeq refGenome markerFile outputDirectory
    
Parameters:

* `refGenome` - a file containing the human reference genome in fasta format
* `markerFile` - A list of all markers for the given chromosome. Format:

        chrom startCoordinate endCoordinate repeatMotif numOfRepeatsInRef

* `outputDirectory` - The script will create a file called chr${num}markerInfo in the directory supplied.

The second script, findFlankingRequired.py, determines the minimum number of flanking bases required on each side of the repeat. 
This is to handle cases where flanking sequences are very similar to the repeat itself and then we require a longer matching sequence on each side

Call:

    findFlankingRequired.py chr${num}markerInfo finalMarkerInfoFile

Parameters:

* `chr${num}markerInfo` - The output file from getRefSeq
* `finalMarkerInfoFile` - File generated by the script. This file has the format required of the markerInfoFile passed to computeReadAttributes
